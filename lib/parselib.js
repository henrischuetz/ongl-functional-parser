...
function notParse(...charsToNotMatch) {
    return (input) => {

        if (input && !charsToNotMatch.includes(input[0])) {
            return resultSet('!' + charsToNotMatch.join(''), input[0], input.substr(1))
        } else {
            return resultSet('!' + charsToNotMatch.join(''), "", input)
        }
    }
}
